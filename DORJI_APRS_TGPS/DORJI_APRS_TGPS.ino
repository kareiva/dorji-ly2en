 ...
 *  Copyright (C) 2022 - Simonas Kareiva - www.github.com/kareiva
 *  
...
char mystatus[128] = "Eterio vagys 2022";
...
SoftwareSerial gps = SoftwareSerial(9,8);
char rmc[100];
int rmc_stat;
...
void DegreesToDegMinSec(float x, char *out, bool lon = false)
{
  int deg=x;
  ...
  if (lon)
    sprintf(out, "%03d%d.%d%c", deg, arcMinutes, arcSeconds, dir);
  else
    sprintf(out, "%02d%d.%d%c", deg, arcMinutes, arcSeconds, dir);
}
...
void loop()
{
  ...
  if (millis() > 5000 && tgps.charsProcessed() < 10)
    Serial.println(F("No GPS data received: check module"));
  else
    displayGPSInfo();

  if (tgps.location.isValid()) {
    DegreesToDegMinSec(tgps.location.lat(), lati, false);
    DegreesToDegMinSec(tgps.location.lng(), lon, true);

    if (tgps.speed.kmph()>5) {
      send_packet(_FIXPOS);
      tx_delay = 30000;
    }
    else
      send_packet(random(4)+1);
  }
  else
     send_packet(_BEACON);
  
  delay(tx_delay);
  randomize(tx_delay, 5000, 60000);
